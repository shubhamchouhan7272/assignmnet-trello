...
const Main = () => {
  ...
  // Edit Task
  const editCard = (item, listIndex) => {
    setIsEditing({ itemId: item.id, listIndex }); 
    setEditValue(item.title); 
  };

  // Update Task
  const updateCard = (listIndex) => {
    let newList = [...bdata.list];
    newList[listIndex].items = newList[listIndex].items.map((item) => {
      if (item.id === isEditing.itemId) {
  ...
  // Delete Task
  const deleteCard = (itemId, listIndex) => {
    let newList = [...bdata.list];
  ...
  return (
    ...
    >
      <div className="p-3 bg-black flex justify-between w-full bg-opacity-50">
        <div className="italic text-2xl font-bold font-sans">
          <h2 className="text-lg">{bdata.name}</h2>
        </div>
        <div className="flex items-center justify-center">
          <button className="bg-white h-8 text-gray-800 px-2 py-1 mr-2 rounded flex justify-center items-center">
      ...
      <div className="flex flex-col w-full flex-grow relative">
        <div className="absolute mb-0 pb-2 left-0 right-0 top-0 bottom-0 p-3 flex overflow-x-scroll overflow-y-hidden">
                ...
                return (
                  ...
                  >
                    <div className="list-body">
                      <div className="flex justify-between p-1 font-semibold bg-gray-600 rounded-md">
                        <span>{x.title}</span>
                                        ...
                                        <div className="item flex justify-between items-center bg-zinc-700 p-1 cursor-pointer rounded-md border-2 border-zinc-900 hover:border-gray-500">
                                          ...
                                          isEditing.itemId === item.id ? (
                                            <input
                                              ...
                                              className="bg-gray-600 p-1 rounded-md w-full"
                                            />
                                          ) : (
                                          ...
                                          <span className="flex justify-start items-start">
                                            ...
                                            isEditing.itemId === item.id ? (
                                              <button
                                                onClick={() => updateCard(ind)}
                                                className="hover:bg-green-600 p-1 rounded-sm"
                                              >
                                                Update
                                              </button>
                                            ) : (
                                              <>
                                                ...
                                                <button
                                                  onClick={() =>
                                                    deleteCard(item.id, ind)
                                                  }
                                                  className="hover:bg-red-600 p-1 rounded-sm ml-2"
                                                >
                                                  <Trash2 size={16} />
                                                </button>
                                              </>
